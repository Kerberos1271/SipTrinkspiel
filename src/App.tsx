...
function App() {
  ...
  return window.location.pathname.startsWith('/admin') ? <AdminApp theme={theme} onToggleTheme={toggleTheme} /> : <PlayerApp theme={theme} onToggleTheme={toggleTheme} />;
}
...
function PlayerApp({ theme, onToggleTheme }: { theme: Theme; onToggleTheme: () => void }) {
  ...
  if (screen === 'setup') {
    return <SetupScreen theme={theme} onToggleTheme={onToggleTheme} data={data} players={players} setPlayers={setPlayers} activeCategoryIds={activeCategoryIds} setActiveCategoryIds={setActiveCategoryIds} onBack={() => setScreen('home')} onStart={startGame} loading={loadState === 'loading'} />;
  }
  if (screen === 'game') return <GameScreen theme={theme} onToggleTheme={onToggleTheme} deck={deck} players={players} onFinish={() => setScreen('finished')} />;
  ...
  return <AppFrame theme={theme} className="setup-screen">
    ...
    <div className="setup-content">
      ...
      <section className="setup-section">
        <div className="section-label"><span>Spieler</span><span className="count-badge">{players.length}</span></div>
        <div className="player-chips">{players.map((player, index) => <div className="player-chip" key={`${player}-${index}`}><span className="avatar" style={{ '--avatar-index': index } as React.CSSProperties}>{player.charAt(0).toUpperCase()}</span><span>{player}</span><button type="button" aria-label={`${player} entfernen`} onClick={() => removePlayer(index)}>×</button></div>)}</div>
        <form className="add-player" onSubmit={(event) => { event.preventDefault(); addPlayer(); }}><input value={name} onChange={(event) => { setName(event.target.value); setError(''); }} placeholder="Name hinzufügen" maxLength={24} aria-label="Spielername" /><button type="submit" aria-label="Spieler hinzufügen">+</button></form>
        {error && <p className="field-error">{error}</p>}
        {players.length < 2 && <p className="field-hint">Noch {2 - players.length} Spieler{2 - players.length === 1 ? '' : 'n'} hinzufügen, dann kann es losgehen.</p>}
      </section>
      <section className="setup-section category-section">
        <div className="section-label"><span>Kategorien</span><span className="category-hint">{cardCount} Karten</span></div>
        <div className="category-list">{data.categories.map((category, index) => <label className={`category-toggle ${activeCategoryIds.includes(category.id) ? 'selected' : ''}`} key={category.id}><input type="checkbox" checked={activeCategoryIds.includes(category.id)} onChange={() => toggleCategory(category.id)} /><span className={`category-marker marker-${colors[index % colors.length]}`} /><span className="category-name">{category.name}</span><span className="checkmark">✓</span></label>)}</div>
      </section>
...
function GameScreen({ theme, onToggleTheme, deck, players, onFinish }: { theme: Theme; onToggleTheme: () => void; deck: Card[]; players: string[]; onFinish: () => void }) {
  ...
  return <AppFrame theme={theme} className="game-screen" onClick={next}>
    <header className="game-header"><Logo light={theme === 'dark'} /><div className="game-header-right"><ThemeToggle theme={theme} onToggle={onToggleTheme} /><span className="round-counter">Card <strong>{index + 1}</strong> / {deck.length}</span><span className="player-count">{players.length} dabei</span></div></header>
...
function AdminDashboard({ theme, onToggleTheme, username, onLogout }: { theme: Theme; onToggleTheme: () => void; username: string; onLogout: () => void }) {
  ...
  useEffect(() => { refresh(); }, []);
  const visibleCards = useMemo(() => filter === 'all' ? cards : cards.filter((card) => String(card.category_id) === filter), [cards, filter]);
  ...
  return <AppFrame theme={theme} className="admin-dashboard"><header className="dashboard-header"><a href="/" aria-label="Zurück zur App"><Logo light={theme === 'dark'} /></a><div className="dashboard-user"><ThemeToggle theme={theme} onToggle={onToggleTheme} /><span>Angemeldet als <strong>{username}</strong></span><button type="button" onClick={logout}>Abmelden</button></div></header><div className="dashboard-content"><div className="dashboard-intro"><div><div className="eyebrow dark"><span /> Dein Backstage</div><h1>Deck<br /><em>Studio.</em></h1></div><div className="dashboard-stats"><div><strong>{categories.length}</strong><span>Kategorien</span></div><div><strong>{cards.length}</strong><span>Karten</span></div></div></div>{(error || notice) && <div className={`dashboard-message ${error ? 'is-error' : ''}`}>{error || notice}<button type="button" onClick={() => { setError(''); setNotice(''); }}>×</button></div>}<nav className="dashboard-tabs" aria-label="Adminbereiche"><button type="button" className={activeTab === 'deck' ? 'active' : ''} onClick={() => setActiveTab('deck')}>Deck verwalten <span>{cards.length}</span></button><button type="button" className={activeTab === 'import' ? 'active' : ''} onClick={() => setActiveTab('import')}>CSV importieren</button></nav>{activeTab === 'import' ? <CsvImportPanel onImport={importCsv} importing={importing} /> : <div className="admin-columns"><section className="admin-card manage-categories"><div className="admin-card-heading"><div><span className="card-kicker">01 / Kategorien</span><h2>Ordnung<br /><em>schaffen.</em></h2></div><span className="heading-icon">✳</span></div><form className="inline-form" onSubmit={addCategory}><input value={categoryName} onChange={(event) => setCategoryName(event.target.value)} maxLength={60} placeholder="Neue Kategorie" /><button type="submit" aria-label="Kategorie hinzufügen">+</button></form><div className="admin-category-list">{categories.map((category, index) => <div className="admin-category-row" key={category.id}><span className={`category-marker marker-${colors[index % colors.length]}`} /><span>{category.name}</span><span className="category-card-count">{cards.filter((card) => card.category_id === category.id).length} Karten</span><button type="button" className="row-delete" onClick={() => deleteCategory(category)} aria-label={`${category.name} löschen`}>×</button></div>)}</div></section><section className="admin-card manage-cards"><div className="admin-card-heading"><div><span className="card-kicker">02 / Karten</span><h2>Die guten<br /><em>Fragen.</em></h2></div><span className="heading-icon">✦</span></div><form className="card-create-form" onSubmit={addCard}><textarea value={cardText} onChange={(event) => setCardText(event.target.value)} maxLength={500} placeholder="Neue Karte schreiben …  (Tipp: ${player} wird im Spiel ersetzt)" rows={3} /><div className="form-row"><select value={selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)} aria-label="Kategorie wählen"><option value="" disabled>Kategorie wählen</option>{categories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select><button className="small-submit" type="submit">Hinzufügen <ArrowIcon /></button></div></form><div className="card-filter"><button type="button" className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>Alle <span>{cards.length}</span></button>{categories.map((category) => <button type="button" className={filter === String(category.id) ? 'active' : ''} onClick={() => setFilter(String(category.id))} key={category.id}>{category.name} <span>{cards.filter((card) => card.category_id === category.id).length}</span></button>)}</div><div className="admin-card-list">{visibleCards.map((card) => <div className="admin-prompt-row" key={card.id}><div><span className="prompt-category">{card.category_name}</span><p>{card.text}</p></div><button type="button" className="row-delete" onClick={() => deleteCard(card)} aria-label="Karte löschen">×</button></div>)}{!visibleCards.length && <p className="empty-state">Noch keine Karten in dieser Ansicht.</p>}</div></section></div>}</div></AppFrame>;
}

function AdminDashboardLegacy({ theme, onToggleTheme, username, onLogout }: { theme: Theme; onToggleTheme: () => void; username: string; onLogout: () => void }) {
  const [categories, setCategories] = useState<Category[]>([]); const [cards, setCards] = useState<Card[]>([]); const [categoryName, setCategoryName] = useState(''); const [cardText, setCardText] = useState(''); const [selectedCategory, setSelectedCategory] = useState(''); const [filter, setFilter] = useState('all'); const [error, setError] = useState(''); const [notice, setNotice] = useState(''); const [importing, setImporting] = useState(false); const [activeTab, setActiveTab] = useState<'deck' | 'import'>('deck');
  const refresh = async () => { try { const [categoryResult, cardResult] = await Promise.all([api<{ categories: Category[] }>('/api/admin/categories'), api<{ cards: Card[] }>('/api/admin/cards')]); setCategories(categoryResult.categories); setCards(cardResult.cards); if (!categoryResult.categories.some((category) => String(category.id) === selectedCategory)) setSelectedCategory(categoryResult.categories[0] ? String(categoryResult.categories[0].id) : ''); } catch (loadError) { setError(loadError instanceof Error ? loadError.message : 'Daten konnten nicht geladen werden.'); } };
  useEffect(() => { refresh(); }, []);
  const visibleCards = useMemo(() => filter === 'all' ? cards : cards.filter((card) => String(card.category_id) === filter), [cards, filter]);
  const addCategory = async (event: React.FormEvent) => { event.preventDefault(); if (!categoryName.trim()) return; try { await api('/api/admin/categories', { method: 'POST', body: JSON.stringify({ name: categoryName }) }); setCategoryName(''); setError(''); setNotice('Kategorie angelegt.'); await refresh(); } catch (addError) { setError(addError instanceof Error ? addError.message : 'Kategorie konnte nicht angelegt werden.'); } };
  const deleteCategory = async (category: Category) => { if (!window.confirm(`Kategorie „${category.name}“ und alle zugehörigen Karten löschen?`)) return; try { await api(`/api/admin/categories/${category.id}`, { method: 'DELETE' }); setNotice('Kategorie und Karten gelöscht.'); await refresh(); } catch (deleteError) { setError(deleteError instanceof Error ? deleteError.message : 'Löschen fehlgeschlagen.'); } };
  const addCard = async (event: React.FormEvent) => { event.preventDefault(); if (!cardText.trim() || !selectedCategory) return; try { await api('/api/admin/cards', { method: 'POST', body: JSON.stringify({ text: cardText, category_id: Number(selectedCategory) }) }); setCardText(''); setError(''); setNotice('Karte angelegt.'); await refresh(); } catch (addError) { setError(addError instanceof Error ? addError.message : 'Karte konnte nicht angelegt werden.'); } };
  const deleteCard = async (card: Card) => { if (!window.confirm('Diese Karte löschen?')) return; try { await api(`/api/admin/cards/${card.id}`, { method: 'DELETE' }); setNotice('Karte gelöscht.'); await refresh(); } catch (deleteError) { setError(deleteError instanceof Error ? deleteError.message : 'Löschen fehlgeschlagen.'); } };
  const importCsv = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; setImporting(true); setError(''); try { const result = await api<{ importedCards: number; skippedRows: number; createdCategories: number }>('/api/admin/import', { method: 'POST', body: JSON.stringify({ csv: await file.text() }) }); setNotice(`${result.importedCards} Karten importiert${result.skippedRows ? `, ${result.skippedRows} fehlerhafte Zeile${result.skippedRows === 1 ? '' : 'n'} übersprungen` : ''}${result.createdCategories ? `, ${result.createdCategories} Kategorien neu angelegt` : ''}.`); await refresh(); } catch (importError) { setError(importError instanceof Error ? importError.message : 'CSV konnte nicht importiert werden.'); } finally { setImporting(false); event.target.value = ''; } };
  const logout = async () => { await api('/api/auth/me', { method: 'POST' }).catch(() => undefined); onLogout(); };
  return <AppFrame className="admin-dashboard"><header className="dashboard-header"><a href="/" aria-label="Zurück zur App"><Logo /></a><div className="dashboard-user"><span>Angemeldet als <strong>{username}</strong></span><button type="button" onClick={logout}>Abmelden</button></div></header><div className="dashboard-content"><div className="dashboard-intro"><div><div className="eyebrow dark"><span /> Dein Backstage</div><h1>Deck<br /><em>Studio.</em></h1></div><div className="dashboard-stats"><div><strong>{categories.length}</strong><span>Kategorien</span></div><div><strong>{cards.length}</strong><span>Karten</span></div></div></div>{(error || notice) && <div className={`dashboard-message ${error ? 'is-error' : ''}`}>{error || notice}<button type="button" onClick={() => { setError(''); setNotice(''); }}>×</button></div>}<section className="csv-import-card"><div className="csv-import-copy"><span className="card-kicker">03 / Import</span><h2>Viele Karten?<br /><em>Einfach rein.</em></h2><p>Importiere ein ganzes Deck als CSV. Fehlende Kategorien werden automatisch angelegt.</p></div><div className="csv-instructions"><p><strong>Format</strong> UTF-8, Trennzeichen <code>;</code>, eine Karte pro Zeile.</p><p><strong>Kopfzeile</strong> muss exakt <code>category_name;card_text</code> lauten.</p><p><strong>Beispiel</strong></p><pre><code>category_name;card_text{`\n`}Fragen;Wer würde am ehesten zuerst lachen?{`\n`}&quot;Gruppenaufgaben&quot;;&quot;Alle stoßen an; die letzte Person trinkt.&quot;</code></pre><p className="csv-note">Semikolons im Inhalt in Anführungszeichen setzen. Anführungszeichen im Inhalt verdoppeln.</p></div><label className="csv-file-button"><input type="file" accept=".csv,text/csv" onChange={importCsv} disabled={importing} /><span>{importing ? 'Import läuft …' : 'CSV auswählen'}</span><ArrowIcon /></label></section><div className="admin-columns"><section className="admin-card manage-categories"><div className="admin-card-heading"><div><span className="card-kicker">01 / Kategorien</span><h2>Ordnung<br /><em>schaffen.</em></h2></div><span className="heading-icon">✳</span></div><form className="inline-form" onSubmit={addCategory}><input value={categoryName} onChange={(event) => setCategoryName(event.target.value)} maxLength={60} placeholder="Neue Kategorie" /><button type="submit" aria-label="Kategorie hinzufügen">+</button></form><div className="admin-category-list">{categories.map((category, index) => <div className="admin-category-row" key={category.id}><span className={`category-marker marker-${colors[index % colors.length]}`} /><span>{category.name}</span><span className="category-card-count">{cards.filter((card) => card.category_id === category.id).length} Karten</span><button type="button" className="row-delete" onClick={() => deleteCategory(category)} aria-label={`${category.name} löschen`}>×</button></div>)}</div></section><section className="admin-card manage-cards"><div className="admin-card-heading"><div><span className="card-kicker">02 / Karten</span><h2>Die guten<br /><em>Fragen.</em></h2></div><span className="heading-icon">✦</span></div><form className="card-create-form" onSubmit={addCard}><textarea value={cardText} onChange={(event) => setCardText(event.target.value)} maxLength={500} placeholder="Neue Karte schreiben …  (Tipp: ${player} wird im Spiel ersetzt)" rows={3} /><div className="form-row"><select value={selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)} aria-label="Kategorie wählen"><option value="" disabled>Kategorie wählen</option>{categories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select><button className="small-submit" type="submit">Hinzufügen <ArrowIcon /></button></div></form><div className="card-filter"><button type="button" className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>Alle <span>{cards.length}</span></button>{categories.map((category) => <button type="button" className={filter === String(category.id) ? 'active' : ''} onClick={() => setFilter(String(category.id))} key={category.id}>{category.name} <span>{cards.filter((card) => card.category_id === category.id).length}</span></button>)}</div><div className="admin-card-list">{visibleCards.map((card) => <div className="admin-prompt-row" key={card.id}><div><span className="prompt-category">{card.category_name}</span><p>{card.text}</p></div><button type="button" className="row-delete" onClick={() => deleteCard(card)} aria-label="Karte löschen">×</button></div>)}{!visibleCards.length && <p className="empty-state">Noch keine Karten in dieser Ansicht.</p>}</div></section></div></div></AppFrame>;
}
